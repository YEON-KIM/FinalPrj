...
<body>
	...
	<div class="container">
		...
		onclick="test()">제발</button>

				<form action="${root}/product/edit" method="POST" enctype="multipart/form-data">
					<input type="hidden" name="no" value="${pvo.no}">
					<input type="hidden" name="getAttNo">
					<script>
						var attList = [<c:forEach items="${pvo.attList}" var="fvo">${fvo.no},</c:forEach>];
					  
						// 배열을 문자열로 변환하여 input 요소에 할당
						var inputElement = document.querySelector('input[name="getAttNo"]');
						inputElement.value = attList.join(',');
					...
					<div class="row">
						...
						<div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" name="name" value="${pvo.name}">
						</div>
						...
						<div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" name="info" placeholder="상품 리스트에 적힐 설명을 적어주세요." value="${pvo.info}">
						</div>

                        <div class="col-md-12 mb-3">
                            <label for="code">상품 상세</label>
							<p>상세 정보는 새롭게 추가해 주세요.</p>
                            <br>
                        </div>
					...
					<div>
					<button class="btn btn-secondary btn-lg btn-block" type="submit">상품 수정</button>
				</form>
					<button class="btn btn-secondary btn-lg btn-block" type="button"
					onclick="locotion.href='${root}/product/detail?num=${pvo.no}'">뒤로가기</button>
				</div>
				</div>
			</div>
